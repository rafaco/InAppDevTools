...
public class BroadcastReceiversScreen extends AbstractFlexibleScreen {

    public BroadcastReceiversScreen(ScreenManager manager) {
        super(manager);
    }
    ...
    private List<Object> getFlexibleData() {
        ...
        data.add(new OverviewData("Broadcast Receivers",
                "Enabled receivers by this application",
                R.string.gmd_hearing,
                R.color.rally_white));

        List<ResolveInfo> runningItems = RunningBroadcastReceiversUtils.getList();
        ...
        if (runningItems.isEmpty()){
            ...
            for (ResolveInfo info : runningItems) {

                String title = RunningBroadcastReceiversUtils.getTitle(info);
                String content = RunningBroadcastReceiversUtils.getContent(info);
                DocumentSectionData.Builder serviceDataBuilder = new DocumentSectionData.Builder(title)
                        //.setIcon(R.string.gmd_view_carousel)
                        .setExpandable(false)
                        .add(content);

                String className = RunningBroadcastReceiversUtils.getClassName(info);
                final String srcPath = getSourcesManager().getPathFromClassName(className);
